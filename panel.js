const PROFILES = [
  ...
  {
    id: "bigo-tv-video-potrait",
    label: "Bigo.tv Video Portrait",
    file: "profiles/bigo-tv-video-potrait.json",
  },
  {
    id: "facebook",
    label: "Facebook",
...
function buildInspectionScript(profile) {
  return [
    ...
    "  const report = { removed: 0, styled: 0, paused: 0, clearedSrc: 0, removedAfter: 0, hiddenAfter: 0, hiddenInstead: 0, missing: [], guardSelectors: 0, styleGuardSelectors: 0, styleGuardActive: false, faviconsDisabled: 0, faviconGuardActive: false, titlesSanitized: 0, titleGuardActive: false, urlsSanitized: 0, urlGuardActive: false };",
    "",
    ...
    "  };",
    "",
    "  const getStyleGuardState = () => {",
    "    if (!window.__webViewProfilerStyleGuardState) {",
    "      window.__webViewProfilerStyleGuardState = {",
    "        rules: [],",
    "        observer: null,",
    "      };",
    "    }",
    "",
    "    return window.__webViewProfilerStyleGuardState;",
    "  };",
    "",
    "  const applyGuardedStyleRule = (rule, options = {}) => {",
    ...
    "    }",
    "",
    "    targets.forEach((element) => applyStyleDeclarations(element, rule.declarations || {}));",
    "    if (countInReport) {",
    "      report.styled += targets.length;",
    "    }",
    "",
    "    return targets.length;",
    "  };",
    "",
    "  const installStyleGuard = (rule) => {",
    "    if (!rule || !rule.selector) {",
    "      return;",
    "    }",
    "",
    "    const guardState = getStyleGuardState();",
    "    const ruleKey = `${rule.selector}::${JSON.stringify(rule.declarations || {})}`;",
    "    const exists = guardState.rules.some((entry) => entry.key === ruleKey);",
    "    if (!exists) {",
    "      guardState.rules.push({",
    "        key: ruleKey,",
    ...
    "    }",
    "",
    "    report.styleGuardSelectors = guardState.rules.length;",
    "",
    "    if (!guardState.observer) {",
    "      guardState.observer = new MutationObserver((mutations) => {",
    "        const activeGuard = getStyleGuardState();",
    "",
    ...
    "          }",
    "",
    "          activeGuard.rules.forEach((guardRule) => {",
    "            try {",
    "              if (element.matches(guardRule.selector)) {",
    "                applyStyleDeclarations(element, guardRule.declarations);",
    "              }",
    "            } catch (_) {",
    "              return;",
    "            }",
    "",
    "            try {",
    "              element.querySelectorAll(guardRule.selector).forEach((match) => {",
    "                applyStyleDeclarations(match, guardRule.declarations);",
    "              });",
    "            } catch (_) {",
    ...
    "",
    "    const activeGuard = getStyleGuardState();",
    "    activeGuard.rules.forEach((guardRule) => applyGuardedStyleRule(guardRule, { countInReport: false }));",
    "    report.styleGuardActive = !!activeGuard.observer;",
    "  };",
    "",
    ...
    "",
    "    if (rule.type === 'style') {",
    "      const styledCount = applyGuardedStyleRule(rule);",
    "      if (rule.observe === true || rule.persistent === true) {",
    "        installStyleGuard(rule);",
    "      }",
    "      if (!styledCount && rule.observe !== true && rule.persistent !== true) {",
    "        report.missing.push(rule.selector);",
    "      }",
    "      return;",
    "    }",
